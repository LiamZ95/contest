...
class MixedAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
	...
	'''
	Using DFS based algorithm to find all the dead end
	'''
	self.deadEnd = list()
	# def DFSExplore(currentState, visited):
	...
	# 		notDeadEnd += 1
	# 	if notDeadEnd == 1:
	# 	  self.deadEnd.append(currentPos)
  ...
  def chooseOffensiveAction(self, gameState):
	...
	if not gameState.getAgentState(self.index).isPacman and self.communication.coordAttakerChasing != None:
	  ...
	  for action in legalActions:
		...
		if not currentPosition in self.deadEnd and newCoor in self.deadEnd:
		#   print 'take ',action,' leads to deadEnd'
		  continue
	...
	while True:
	  ...
	  stateIndex = stateIndex % len(self.simulationResult)
  ...
  def chooseAction(self, gameState):
	...
	# if currpos in self.deadEnd:
	#   print 'agent ',self.index,' is in dead end'
	# else:
	#   print 'agent ',self.index,' is not in dead end'
