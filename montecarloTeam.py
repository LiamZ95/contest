...
class MonteCarloFactory(AgentFactory):
    """
    Initiate agent, like captureAgent
    """
...
class Attacker(EvaluationBasedAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if len(inRange) > 0:
      ...
      # This feature only valid when enemy is within 5 to our agent
      if closestDist <= 5:
  ...
  def getWeights(self, gameState, action):
    ...
    # Avoid the situation that pacman change between ghost and pacman over and over again
  ...
  def randomSimulation(self, depth, gameState):
    ...
    # Depth is used to control exploration height
    while depth > 0:
      # Get valid actions
  ...
  def takeToEmptyAlley(self, gameState, action, depth):
    ...
    new_state = gameState.generateSuccessor(self.index, action) # Successor
    ...
    # If no action available, then is deadend
    if len(actions) == 0:
      return True
  ...
  def chooseAction(self, gameState):
    ...
    for a in all_actions:
      if not self.takeToEmptyAlley(gameState, a, 5):
        actions.append(a)
    # If no actions meet the condition, just use all available actions
    if len(actions) == 0:
      actions = all_actions
...
class Defender(CaptureAgent):
  ...
  # Randomly select pos in patrolDict
  ...
  def registerInitialState(self, gameState):
    ...
    # Select only half the spots around centerX to patrol
    self.noWallSpots = []
